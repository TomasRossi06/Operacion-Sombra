...
#    3. Sistema de profiles y menu (lineas ~270 en adelante)
#    4. Main(al final)
# =============================================================================
...
def pause():
    """
    | Descripcion: Detiene la ejecucion hasta que el player presione ENTER. Sirve para que el player pueda leer los mensajes en pantalla
...
def create_profile():
    """
    | Descripcion: Menu para crear un nuevo perfil de player. Valida que el name no este vacio ni repetido.
    ...
    if name == "":
        print("  [!] El name no puede estar vacio.")
        pause()
        return
...
def select_user():
    """
    | Descripcion: Muestra la lista de profiles existentes y pide al player que elija uno por numero
    | Entrada: Seleccion numerica ingresada por teclado
    | Salida: string con el name del perfil elegido si la seleccion es valida.
    |         False si el player cancela (elige 0) o ingresa algo invalido
    """
    if not profiles:
        print("\n  [!] No hay profiles registrados. Crea uno primero.")
        pause()
        return False
...
def start_game():
    """
    | Descripcion: Permite al player elegir un level e iniciarlo. Solo muestra los niveles disponibles segun el progreso del player.
    |              Un level se desbloquea al completar el anterior.
    | Entrada: Seleccion de usuario y level ingresada por teclado.
    | Salida: No retorna nada. Llama a la funcion del level elegido y actualiza el progreso del player segun el resultado.
    """
    clean()
    print(BANNER)
    print("  +-----------------------------+")
...
def access_secret_code():
    """
    | Descripcion: Permite ingresar un code secreto para desbloquear todos los niveles de un perfil sin tener que completarlos.
    | Entrada: Nombre de usuario (elegido de la lista) y code secreto por teclado.
    | Salida: Modifica el perfil del player si el code es correcto. No retorna ningun valor.
    """
    clean()
    print(BANNER)
    print("  +-----------------------------+")
    ...
    code = input("\n  Ingresa el code secreto: ").strip().upper()
...
def update_ranking(name, level):
    """
    | Descripcion: Registra el score obtenido por un player al completar un level.
    |              Si el player no tiene usinput en el ranking, la crea. El score se calcula como nivelx100.
    | Entrada: name -> string con el name del player (ej: "Juan")
    |          level  -> entero con el numero de level completed (ej: 2)
...
def main_menu():
    """
    | Descripcion: Muestra el menu principal y redirige al player segun su choice.
